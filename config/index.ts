  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('site');

        const virtualModuleId = 'global:config';
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Project \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Project config has been loaded.`);
        }
      },
